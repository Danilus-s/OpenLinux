...
local keyBindHandlers = {
  ...
  save = function()
    ...
    if f then
      ...
      f:close()
      if require("perm").save(filename) then
        local txt = ""
        for t in io.lines(filename) do
          txt = txt .. t .. "\n"
        end
        local needL
        ...
        for l in io.lines("/etc/sys/sums") do
            prof[#prof + 1] = l
            if string.find(l, filename) ~= nil then
                needL = #prof
            end
        end
        if needL == nil then needL = #prof+1 end
        prof[needL] = filename .. ":" .. require("sha2").md5(txt)
        ...
        local file = io.open("/etc/sys/sums", "w")
