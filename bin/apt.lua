...
local function install(fi, gt, y)
  ...
  if fs.exists("/etc/apt/installed.list") and not fs.isDirectory("/etc/apt/installed.list") then
    local inst = {}
    local f = io.open("/etc/apt/installed.list")
    while true do
      local t = f:read()
      if t ~= nil then
        inst[#inst + 1] = t
        if adv.split(t, ":")[1] == gt[1] then f:close() goto skip; break end
      else
        break
      end
    end
  ...
  local f = io.open("/etc/apt/installed.list", "a")
  f:write(gt[1] .. ":" .. gt[2] .. "\n")
  f:close()
  ::skip::
end
...
elseif args[1] == "upgrade" then
  if #args == 2 then
      local get
      for l in io.lines("/etc/apt/installed.list") do
        if string.lower(adv.split(l, ":")[1]) == string.lower(args[2]) then
          get = adv.split(l, ":")
          break
        end
...
elseif args[1] == "remove" and args[2] ~= nil then
  ...
  for i = 1, #files do
    if fs.exists(adv.split(files[i], ":")[3]) and not fs.isDirectory(adv.split(files[i], ":")[3]) and adv.split(files[i], ":")[1] == "file" then
  ...
  while true do
    local t = f:read()
    if t ~= nil then
        if string.lower(adv.split(t, ":")[1]) == string.lower(args[2]) then goto skip end
        inst[#inst + 1] = t
        ::skip::
    else
      break
  ...
  f:close()
  f = io.open("/etc/apt/installed.list", "w")
  for i = 1, #inst do
    f:write(inst[i])
  end
  f:close()
